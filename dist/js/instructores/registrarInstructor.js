document
  .querySelector("#btn_registro_instructor")
  .addEventListener("click", (e) => {
    ...
    `,
      ...
      didOpen: () => {
        document
          .querySelector("#frm_registro_instructor")
          .addEventListener("submit", function (e) {
            e.preventDefault();
            const formData = new FormData(this);
            const nombre = document
              .querySelector("#nombre_instructor")
              .value.trim();
            const apellido = document
              .querySelector("#apellido_instructor")
              .value.trim();

            const email = document.querySelector("#correo_instructor").value.trim();

            const contrasena = document
              .querySelector("#pass_instructor")
              .value.trim();
            ...
            async function registroUsuario() {
              const respuesta = await fetch(
                "../controller/instructores/agregarInstructor.php",
                {
                  method: "POST",
                  body: formData,
                }
