function editarInstructor(btn) {
    ...
    Swal.fire({
        title: '<h3 class="fw-bold mb-3 text-primary">Editar información del Instructor</h3>',
        html: `
    ...
    `,
        ...
        didOpen: () => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const respuesta = await fetch('../controller/instructores/editarInstructor.php', {
                        method: 'POST',
                        body: formData,
                    });
                    ...
                    } else if (res.includes('Esté correo ya está registrado en la base de datos!')) {
                        Swal.fire('Fallo', res, 'question');
                    } else if (res.includes('Ya existe un instructor con ese correo')) {
                        Swal.fire('Fallo', res, 'question');
                    } else {
                        Swal.fire('Error', res || 'No se pudo actualizar el usuario.', 'error');
                    }
                } catch (error) {
                    console.error(error);
